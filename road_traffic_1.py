...
min_area = 50/(360*640)
max_area = 2000/(360*640)

print(first_frame.shape)
h,w = first_frame.shape

line = np.array([(w*0.4,h*0.77),(w*0.656,h*0.77)], dtype=int)
line_2 = np.array([(w*0.4,h*0.83),(w*0.656,h*0.83)], dtype=int)

polly = np.array([(w*0.44,h*0.447), (w*0.375,h*0.91), (w*0.650,h*0.9), (w*0.64,h*0.5)], dtype=int)
...
while 1:
    ...
    bgmask = cv2.morphologyEx(bgmask, cv2.MORPH_DILATE, structuring_element,iterations=1)
    rects = get_bbox(bgmask, min_area, max_area)
    if len(rects)!=0:
        ratio = (rects[:,-2:].min(axis=1)/rects[:,-2:].max(axis=1))
        rects = rects[ratio>0.4]
    ...
    cv2.line(color_frame,line[0], line[1], (0,0,255), 2)
    ...
    if frame_counter>total_frames-1 or frame_counter%1000==0:
        cv2.imwrite('road_traffic_1_count.png',color_frame)
